...
public class JdbcTemplateTest {
	...
	void insert2() {
		sql.append("insert into student (id,name,kor,eng,math) ");
		sql.append(" values ('id3','name3',50,60,70)");
	...
	void insert4() {
		...
		cnt = jt.update(sql.toString(), "id5","name5",90,100,100);
		logger.info("생성건수 : " + cnt);
	}
	...
	void insert7() {
		...
		cnt = jt.update(
				sql.toString(),
				new Object[] {"id9","name9",100,100,100 },
				ty
		);
...
/*		for(Map<String,Object> map : list) {
	...
	void list2() {
		...
		/* 1개 레코드의 1개 컬럼값만을 기본타입으로 List로 담아올때 사용 */
...
//		logger.info(map.toString());
	...
	void list11() {
		...
		logger.info("국어점수가 50이상이고 수학점수가 60이상인 학생수 :" + cnt);
	}	
	...
	@Test @Disabled
	void list13() {
		sql.append("select id,name,kor,eng,math from student where name = 'name1'");

		
		Student2 s = null;
		
		// 한행의 테이블 레코드와 객체를 수동으로매핑해줄때 사용 (전제조건 : 테이블 컬럼명과 객체의 멤버이름이 다를경우)
		s = jt.queryForObject(
				sql.toString(), 
